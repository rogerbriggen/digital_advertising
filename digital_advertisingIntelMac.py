...
from torchrl.data.tensor_specs import (
    OneHotDiscreteTensorSpec as OneHot,
    BoundedTensorSpec as Bounded,
    UnboundedContinuousTensorSpec as Unbounded,
    CompositeSpec as Composite, BinaryDiscreteTensorSpec
)
...
class AdOptimizationEnv(EnvBase):
    ...
    def __init__(self, dataset, initial_cash=100000.0, device="cpu"):
        ...
        self.done_spec = Composite(
            done=BinaryDiscreteTensorSpec(n=1, shape=(1,), dtype=torch.bool),
            terminated=BinaryDiscreteTensorSpec(n=1, shape=(1,), dtype=torch.bool),
            truncated=BinaryDiscreteTensorSpec(n=1, shape=(1,), dtype=torch.bool),
        )
...
def create_policy(env, feature_dim, num_keywords, device):
    ...
    policy = TensorDictSequential(flatten_module, value_net, QValueModule(action_space=env.action_spec))
