...
def analyze_training_progress(metrics, output_dir):
    ...
    if loss_metric:
        ...
        loss_df['smoothed_value'] = loss_df['value'].rolling(window=10, min_periods=1).mean()
    ...
    if loss_metric and performance_metric:
        ...
        common_steps = sorted(list(set(loss_df['step'].tolist()).intersection(set(perf_df['step'].tolist()))))
        
        if common_steps:
            # Filter data to common steps
            filtered_loss = loss_df[loss_df['step'].isin(common_steps)]
            filtered_perf = perf_df[perf_df['step'].isin(common_steps)]
            
            # Normalize
            normalized_loss = scaler.fit_transform(filtered_loss[['value']]).flatten()
            normalized_perf = scaler.fit_transform(filtered_perf[['value']]).flatten()
            
            # Correlation analysis
            plt.figure(figsize=(10, 8))
            plt.scatter(normalized_loss, normalized_perf, alpha=0.7)
            plt.title('Correlation Between Loss and Performance')
            plt.xlabel('Normalized Loss')
            plt.ylabel('Normalized Performance')
            plt.grid(True, alpha=0.3)
            
            # Add trend line
            z = np.polyfit(normalized_loss, normalized_perf, 1)
            p = np.poly1d(z)
            plt.plot(normalized_loss, p(normalized_loss), "r--")
            
            # Add correlation coefficient
            corr = np.corrcoef(normalized_loss, normalized_perf)[0, 1]
            plt.annotate(f'Correlation: {corr:.2f}', 
                         xy=(0.05, 0.95), 
                         xycoords='axes fraction',
                         bbox=dict(boxstyle="round,pad=0.3", fc="white", alpha=0.8))
            
            corr_plot_path = os.path.join(output_dir, "loss_performance_correlation.png")
            plt.savefig(corr_plot_path, dpi=300, bbox_inches="tight")
            plt.close()
            plots.append(corr_plot_path)
